...
public class DFS extends GraphAlgorithm<DFS.DFSVertex> {
	...
	static class DFSVertex {
		...
		/**
		 * This function is to check if v is an ancestor of u in DFS tree or not. If v
		 * is an ancestor of u then that means existence of a back edge.
		 * 
		 * @param v
		 *            - a vertex of graph G
		 * @return - true if v is an ancestor of u, false otherwise
		 */
	...
	void DFSVisit(Graph.Vertex u) {
		...
		while (outEdgeItr.hasNext()) {
			...
			if (!seen(v)) {
				...
				// If there is a back edge in the graph, then there is a cycle an thus the graph
				// is not a DAG
