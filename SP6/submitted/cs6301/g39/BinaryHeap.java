...
public class BinaryHeap<T> {
	...
	// Helper method for deleteMin(). "Minimum" element is always at index 0, i.e.
	// at root
	public T remove() {
		if (size == 0)
			throw new ArrayIndexOutOfBoundsException();
	...
	/*
	 * Restore heap order in direction from leaf node to the root. pq[i] may violate
	 * heap order with parent
	 */
	...
	/*
	 * Restore heap order in direction from root to leaf node. pq[i] may violate
